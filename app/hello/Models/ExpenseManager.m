...
- (NSArray *)loadExpensesOfDay:(NSDate *)date orderBy:(NSString *)fieldName ascending:(BOOL)isAscending {
    ...
    NSString* sqlText = [NSString stringWithFormat:@"select ExpenseId, Amount, CategoryId, substr(Date, 0, 11) as Date, Notes, PictureRef, UseLocation, Latitude, Longitude from expense where Date = %@ order by %@ %@", dateStr, fieldName, optionStr];
...
- (NSArray*)getExpensesBetween:(NSDate *)startDate endDate:(NSDate *)endDate orderBy:(NSString *)fieldName assending :(BOOL)assending {
    ...
    NSString* sql = [NSString stringWithFormat:@"select ExpenseId, Amount, CategoryId, substr(Date, 0, 11) as Date, Notes, PictureRef, UseLocation, Latitude, Longitude from expense where Date >= %@ and date <= %@", start, end];
    if (fieldName) {
        NSString* order = @"DESC";
        if (assending)
            order = @"ASC";
...
- (NSArray *)getExpensesOfCategory:(int)categoryId fromDate:(NSDate *)startDate toDate:(NSDate *)endDate {
    NSString* start = formatSqlDate(startDate), * end = formatSqlDate(endDate);
    NSString* sql = [NSString stringWithFormat:@"select ExpenseId, Amount, CategoryId, substr(Date, 0, 11) as Date, Notes, PictureRef, UseLocation, Latitude, Longitude from expense where categoryId = %d and date >= %@ and Date <= %@ order by Date desc", categoryId, start, end];
...
- (Expense *)getExpenseById:(NSInteger)expenseId {
    NSString* sql = [NSString stringWithFormat:@"select ExpenseId, Amount, CategoryId, substr(Date, 0, 11) as Date, Notes, PictureRef, UseLocation, Latitude, Longitude from expense where expenseid = %d", expenseId];
    NSMutableArray* array = [NSMutableArray array];
...
+ (NSDate *)firstDayOfExpense {
    NSString* sql = @"select min(substr(date, 0, 11)) MinDate from expense";
